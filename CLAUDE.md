...
- 모든 초안은 `90_System/80_Lua_Details/05_Personal Studio/_Drafts/`에만 두고, 직접 발행하거나 전송하지 않는다.
- `00_Lua/02_Memory/Identity/` 아래 파일을 바꾸는 작업은 파일마다 사람의 명시적 확인이 있어야 한다.
- 예산 상한과 지출 원칙은 `decision-principles.md`에 따른다.
- `_meta/`와 `_System/`은 모든 agent에게 off-limits이다.

## Vault permissions

[[90_System/80_Lua_Details/01_Command Center/04_Policies/_System/agent-permissions]]
