...
const INBOX_REL = path.join('09_Automations', 'Telegram Command Inbox.md');
const INBOX = path.join(ROOT, INBOX_REL);
...
function usage() {
  console.log(`Usage:
  node scripts/telegram_command_inbox.js "/lua research brief :: 테크인 조사"
  node scripts/telegram_command_inbox.js --source telegram-mobile "/lua inbox 밖에서 떠오른 아이디어"
`);
}

function parseArgs(argv) {
  const args = { source: 'telegram-manual', text: null, now: null, root: ROOT };
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === '--source') args.source = argv[++i] || 'telegram-manual';
...
function appendQueue(entry, options = {}) {
  ...
  const marker = '| example-001 | Telegram | `/lua inbox` | 예시 아이디어 | done | [[00_Inbox/AI 분류 대기중...|Inbox]] |';
...
function queueTelegramCommand(options) {
  const parsed = parseCommand(options.text);
  const id = `telegram-${todayStamp(options.now || new Date())}`;
  appendQueue({ id, source: options.source || 'telegram-manual', ...parsed }, { root: options.root || ROOT });
  return { id, ...parsed };
}
...
function main() {
  ...
  const result = queueTelegramCommand(args);
  console.log(`Queued ${result.id}: ${result.command} -> ${result.payload}`);
}
