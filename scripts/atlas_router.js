...
const ACTION_BOARD_REL = path.join('01_Command Center', '00_Dashboard', 'User Action Board.md');
...
- 추천 이유: 명령 작성기는 작고, 지금 Lua 시스템의 약점인 "입력 난이도"를 바로 줄인다.`,
    plan: `- [x] 첫 처리 대상: [[01_Command Center/01_Commands/Command Runs/${path.basename(runRelFor(entry))}|${entry.id}]].
...
| 항목 | 내용 |
|---|---|
| 처리한 command | [[01_Command Center/01_Commands/Command Runs/${path.basename(runRelFor(entry))}|${entry.id}]] |
