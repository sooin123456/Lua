---
ai-index: true
type: hub
area: Energy Policies
last_updated: 2026-05-16
---

# Energy Policies Hub

에너지 정책, 재생에너지 시장, 공공 발주, 경쟁사/협력사 리서치를 모으는 허브다.

## Floating Solar

| Note | Status | Focus |
|---|---|---|
| [[00_Lua/04_Resources/Energy Policies/K-water 수상태양광 Research Brief|K-water 수상태양광 Research Brief]] | draft | K-water 발주 방향, 협력사, 경쟁사, 테크윈/테크윈에너지 |

## Navigation

- [[00_Lua/04_Resources/Resources Hub|Resources Hub]]
- [[00_Lua/01_Command Center/01_Commands/Obsidian Command Center|Obsidian Command Center]]
- [[00_Lua/01_Command Center/00_Dashboard/User Action Board|User Action Board]]
