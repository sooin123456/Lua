...
전체 자동화 흐름은 [[Lua End-to-End Flow|Lua End-to-End Flow]]를 기준으로 본다. 기본 원격 명령 하달 장소는 [[90_System/09_Automations/Telegram Command Inbox|Telegram Command Inbox]]이고, 컴퓨터 앞에서 구조화해 적는 명령 장소는 [[00_Lua/01_Command Center/01_Commands/Obsidian Command Center|Obsidian Command Center]]다.
...
무엇을 Obsidian에 써야 하는지 애매하면 [[00_Lua/01_Command Center/04_Policies/Obsidian Writing Rules|Obsidian Writing Rules]]를 먼저 본다.
명령을 컴퓨터 앞에서 내리는지, 밖에서 Telegram으로 내리는지 애매하면 [[00_Lua/01_Command Center/01_Commands/Command Modes|Command Modes]]를 먼저 본다.

실사용 기준은 [[00_Lua/01_Command Center/04_Policies/Lua Operating Layers|Lua Operating Layers]]를 따른다:

- 기억할 것은 [[00_Lua/01_Command Center/02_Memory/Lua Memory Classification|Memory Classification]]
- 실행할 것은 [[00_Lua/01_Command Center/01_Commands/Lua Command Taxonomy|Command Taxonomy]]
- 끝난 일은 [[00_Lua/01_Command Center/03_Summaries/Lua Activity Summary System|Activity Summary System]]

## Where To Write

| 상황 | 쓰는 곳 | 이유 |
|---|---|---|
| 갑자기 떠오른 아이디어 | [[00_Lua/00_Inbox/AI 분류 대기중...|AI 분류 대기중]] 또는 새 Inbox 노트 | 아직 프로젝트가 아니므로 임시 보관 |
| 오늘 한 일 | [[00_Lua/05_Personal Studio/Daily Notes|Daily Notes]] | 개인 작업 흐름 기록 |
| 회사/프로젝트 진행 | 각 프로젝트의 `DevLog.md` | 나중에 팀 공유와 회고에 사용 |
| 해야 할 일 | 각 프로젝트의 `Backlog.md` 또는 [[00_Lua/01_Command Center/01_Commands/Urgent List|Urgent List]] | 실행 항목만 모으기 |
| 자료 조사 | 각 프로젝트의 `Research.md` | 출처와 판단 근거 보관 |
| 팀에게 보낼 글 | [[00_Lua/03_Operation/Operation Hub|Operation Hub]] 아래 관련 문서 | Slack/Notion 전 검토 공간 |
| Notion/Slack 공유 초안 | [[00_Lua/03_Operation/Team Brief Drafts|Team Brief Drafts]] | 승인 전 초안 보관 |
| 재사용 가능한 결과물 | [[90_System/08_Artifacts/Artifact Ledger|Artifact Ledger]] | 앱, 프롬프트, 템플릿, 자동화 자산화 |
| 시스템 규칙 변경 | [[00_Lua/01_Command Center/03_Summaries/Work Ledger|Work Ledger]] | 어디서 누가 바꿨는지 추적 |
...
| Obsidian 안에 다음 실행 예약 | [[00_Lua/01_Command Center/00_Dashboard/Agent Dashboard|Agent Dashboard]]의 Quick Commands | `/project-sprint time-management-app first-demo` |
| 업무 도메인별 명령 하달 | [[00_Lua/01_Command Center/01_Commands/Obsidian Command Center|Obsidian Command Center]] | `/lua planning prioritize :: 이번 주 우선순위` |
| 프로젝트별 실행 큐 작성 | 프로젝트 `Backlog.md` | `/team-brief green-building` |
| 커밋/출처 기록 | [[00_Lua/01_Command Center/03_Summaries/Work Ledger|Work Ledger]] | `/work-log windows-codex cleanup` |
...
1. [[00_Lua/01_Command Center/00_Dashboard/Master Dashboard|Master Dashboard]]를 연다.
2. 오늘 처리할 프로젝트 Home 또는 Daily Notes를 연다.
3. 아직 정리 안 된 생각은 Inbox/Daily Notes에 먼저 적는다.
4. 바로 실행할 명령은 [[00_Lua/01_Command Center/01_Commands/Obsidian Command Center|Obsidian Command Center]]에 적는다.
5. 실행이 필요하면 Codex나 Claude에 말한다.
6. 결과가 나오면 프로젝트 DevLog, Artifact Ledger, Work Ledger 중 맞는 곳에 남긴다.

## Inbox To Command Flow

Inbox에 추가해도 전체 플로우는 이어진다.

```text
00_Lua/00_Inbox 원문
→ /inbox-triage
→ domain 분류
→ Command Center Queue 또는 프로젝트 노트로 이동
→ Atlas CEO 라우팅
→ agent workflow
→ Obsidian 결과 저장
```
...
- https://pi.dev/90_System/docs/latest
...
- [[00_Lua/01_Command Center/04_Policies/Obsidian Writing Rules|Obsidian Writing Rules]]
...
- [[00_Lua/04_Resources/Tech Stack/Pi Coding Agent|Pi Coding Agent]]
