...
| Node | installed through Codex | enough for `node 90_System/scripts/check.js` |
...
```bash
git status --short
node 90_System/scripts/check.js
node 90_System/scripts/vault_audit.js
```
...
After meaningful sessions, append an entry to `00_Lua/01_Command Center/03_Summaries/Work Ledger.md`.
