---
ai-index: true
type: backlog
area: KIEREMS
project: RTU Development
status: active
---

# 📚 RTU Development Backlog

## 🔗 관련 노트

- [[90_System/80_Lua_Details/02_Projects/KIEREMS/RTU Development/Home|Home]]
- [[90_System/80_Lua_Details/02_Projects/KIEREMS/RTU Development/DevLog|DevLog]]
- [[90_System/80_Lua_Details/01_Command Center/00_Dashboard/Master Dashboard|Master Dashboard]]
- [[90_System/80_Lua_Details/01_Command Center/00_Dashboard/Agent Dashboard|Agent Dashboard]]

## P0 (이번 주 필수)

- [ ] 핵심 부품 대체 벤더 2개 확보
- [ ] 부품 리드타임 재산정 및 납기 리스크 문서화
- [ ] 현지 전압/주파수 검증 테스트 항목 확정

## P1 (다음 순위)

- [ ] RTU 펌웨어 안정성 테스트 시나리오 정리
- [ ] 현지 설치 체크리스트 초안 작성
- [ ] 이슈 재발 방지용 공급망 모니터링 루틴 설계

## Parking Lot

- [ ] 현지 운영 매뉴얼 다국어 버전
- [ ] 유지보수 대시보드 요구사항 정리

## Navigation

- [[90_System/80_Lua_Details/02_Projects/KIEREMS/RTU Development/Home|Home]]
- [[90_System/80_Lua_Details/02_Projects/Projects Hub|Projects Hub]]
