...
- `90_System/scripts/sync_skills.js` — financial-services 패턴의 skill 동기화
- `90_System/scripts/check.js` 확장 (agents 검증)
...
- [ ] **Step 20** — `90_System/scripts/sync_skills.js`
- [ ] **Step 21** — `90_System/scripts/check.js` 확장 (agents 검증)
...
**위치**: `/90_System/07_Lua_System/verticals/climate-energy/README.md`
...
**위치**: `/90_System/07_Lua_System/verticals/climate-energy/.mcp.json`
...
**위치**: `/90_System/07_Lua_System/verticals/climate-energy/skills/proposal-drafting/SKILL.md`
...
**위치**: `/90_System/07_Lua_System/verticals/climate-energy/skills/patent-filing/SKILL.md`
...
**위치**: `/90_System/07_Lua_System/agents/README.md`
...
검증: `node 90_System/scripts/check.js` (drift 검출).
```
...
**위치**: `/90_System/07_Lua_System/agents/atlas/system-prompt.md`
...
**위치**: `/90_System/scripts/sync_skills.js`
...
const AGENTS_DIR = path.join(ROOT, '90_System/07_Lua_System/agents');
const VERTICALS_DIR = path.join(ROOT, '90_System/07_Lua_System/verticals');
...
**Validation**:
- [ ] `node 90_System/scripts/sync_skills.js` 실행됨
- [ ] 각 agent의 `skills/` 하위에 depends_on skill 사본 생김
- [ ] check.js로 sync 후 drift 없음 확인

---

### Step 21 — 90_System/scripts/check.js 확장

**추가 검증**:

```javascript
async function checkAgents() {
  const agentDirs = fs.readdirSync(path.join(ROOT, '90_System/07_Lua_System/agents'))
    .filter(name => {
      const p = path.join(ROOT, '90_System/07_Lua_System/agents', name);
      return fs.statSync(p).isDirectory() && name !== 'README.md';
    });
  
  if (agentDirs.length === 0) {
    warnings.push('No agents found in 90_System/07_Lua_System/agents/');
    return;
  }
  ...
  for (const agent of agentDirs) {
    const promptPath = path.join(ROOT, '90_System/07_Lua_System/agents', agent, 'system-prompt.md');
    if (!fs.existsSync(promptPath)) {
...
async function checkSkillDrift() {
  ...
  const agentSkills = await glob('90_System/07_Lua_System/agents/*/skills/*/SKILL.md', { cwd: ROOT });
  
  for (const bundled of agentSkills) {
    // Extract skill name
    const skillName = path.basename(path.dirname(bundled));
    // Find source in verticals/
    const sources = await glob(`90_System/07_Lua_System/verticals/*/skills/${skillName}/SKILL.md`, { cwd: ROOT });
    if (sources.length === 0) {
...
경로는 `90_System/07_Lua_System/verticals/` 기준 상대 경로. `verticals/` prefix 빼고.
...
2. [ ] `node 90_System/scripts/sync_skills.js` → 모든 agent의 skills/ 동기화됨
3. [ ] `node 90_System/scripts/vault_audit.js` 여전히 동작
