...
`90_System/07_Lua_System/` 골격 + `CLAUDE.md` + `verticals/_core/skills/voice-mimicry/` 완전 구현.

이 phase가 끝나면:
- `anthropics/skills`의 progressive disclosure 패턴 적용 완료
- `anthropics/financial-services`의 verticals + 90_System/scripts/check 패턴 적용 완료
...
- [ ] **Step 4** — `90_System/07_Lua_System/verticals/_core/README.md`
- [ ] **Step 5** — `90_System/07_Lua_System/verticals/_core/.mcp.json`
...
- [ ] **Step 10** — `90_System/scripts/check.js`
...
```bash
mkdir -p 90_System/07_Lua_System/agents
mkdir -p 90_System/07_Lua_System/verticals/_core/skills/voice-mimicry/references
mkdir -p 90_System/07_Lua_System/verticals/_core/skills/voice-mimicry/assets
mkdir -p 90_System/07_Lua_System/verticals/_core/commands
mkdir -p 90_System/07_Lua_System/verticals/climate-energy
mkdir -p 90_System/07_Lua_System/verticals/personal-venture
mkdir -p 90_System/07_Lua_System/managed-agents
mkdir -p .claude-plugin
mkdir -p .github/workflows
mkdir -p scripts
```

**Validation**: `find 90_System/07_Lua_System -type d | sort`로 모든 디렉토리 보이는지 확인.
...
**위치**: `/90_System/07_Lua_System/verticals/_core/README.md`
...
**위치**: `/90_System/07_Lua_System/verticals/_core/.mcp.json`
...
**위치**: `/90_System/07_Lua_System/verticals/_core/skills/voice-mimicry/SKILL.md`
...
async function checkSkills() {
  const skillFiles = await glob('90_System/07_Lua_System/verticals/*/skills/*/SKILL.md', { cwd: ROOT });
  
  if (skillFiles.length === 0) {
    warnings.push('No SKILL.md files found under 90_System/07_Lua_System/verticals/*/skills/');
    return;
  }
...
async function checkMCPConfigs() {
  const mcpFiles = await glob('90_System/07_Lua_System/verticals/*/.mcp.json', { cwd: ROOT });
...
**Validation**:
- [ ] `node 90_System/scripts/check.js` 실행됨
  ...
  check:
    ...
    steps:
      ...
      - name: Run vault check
        run: node 90_System/scripts/check.js
```
...
- 코드 예시는 `references/examples.md` 또는 `90_System/scripts/{tool}.js`로
...
1. [ ] `node 90_System/scripts/check.js` → "✓ All checks passed"
2. [ ] GitHub Actions에서 workflow 자동 실행됨
3. [ ] Claude Desktop에서 Obsidian vault에 접근 가능 (mcp-obsidian 동작)
4. [ ] Cursor에서 `90_System/07_Lua_System/verticals/_core/skills/voice-mimicry/SKILL.md` 열어보고 frontmatter + 7개 섹션 다 있는지 눈으로 확인
...
- `90_System/scripts/sync_skills.js` — vertical skills → agent bundles 자동 동기화
...
| check.js 확장 | financial-services의 90_System/scripts/check.py |
