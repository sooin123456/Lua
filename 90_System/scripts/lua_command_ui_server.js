...
const PROTOTYPE_DIR = path.join(ROOT, '90_System/08_Artifacts', 'Lua Command UI Prototype');
...
function usage() {
  console.log(`Usage:
  node 90_System/scripts/lua_command_ui_server.js
  node 90_System/scripts/lua_command_ui_server.js --port 8765
`);
}
...
function runCommandFromUi(options = {}) {
  ...
  const builtResult = runBuildRunner({
    ...
    apply: true,
    commandId: queued.id,
    verification: options.verification || ['node 90_System/scripts/check.js'],
  });
