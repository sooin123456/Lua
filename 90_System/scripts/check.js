...
function isSkillFile(fullPath) {
  ...
  return rel.startsWith('90_System/07_Lua_System/verticals/')
    && rel.includes('/skills/')
...
function isBundledSkillFile(fullPath) {
  const rel = relativePosix(ROOT, fullPath);
  return rel.startsWith('90_System/07_Lua_System/agents/')
    && rel.includes('/skills/')
    && rel.endsWith('/SKILL.md');
}
...
function isMcpConfig(fullPath) {
  const rel = relativePosix(ROOT, fullPath);
  return rel.startsWith('90_System/07_Lua_System/verticals/') && rel.endsWith('/.mcp.json');
}

function verticalSkillSources(skillName) {
  return findFiles(under('90_System/07_Lua_System', 'verticals'), (fullPath) => {
    const rel = relativePosix(ROOT, fullPath);
...
async function checkSkills() {
  const skillFiles = findFiles(under('90_System/07_Lua_System', 'verticals'), isSkillFile)
    .map((file) => relativePosix(ROOT, file));
...
async function checkCommands() {
  const commandFiles = findFiles(under('90_System/07_Lua_System', 'verticals'), isCommandFile)
    .map((file) => relativePosix(ROOT, file));
...
async function checkMCPConfigs() {
  const mcpFiles = findFiles(under('90_System/07_Lua_System', 'verticals'), isMcpConfig)
    .map((file) => relativePosix(ROOT, file));
...
async function checkAgents() {
  const agentsRoot = path.join(ROOT, '90_System/07_Lua_System/agents');
  if (!fs.existsSync(agentsRoot)) {
    warnings.push('90_System/07_Lua_System/agents/ missing');
    return;
  }
  const agentDirs = fs
...
async function checkSkillDrift() {
  const bundledFiles = findFiles(under('90_System/07_Lua_System', 'agents'), isBundledSkillFile)
    .map((file) => relativePosix(ROOT, file));
