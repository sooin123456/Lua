...
`
  ...
  write(
    path.join(root, '90_System/08_Artifacts', 'Artifact Ledger.md'),
    `---
...
test('build runner turns a planned build command into a done artifact', () => {
  ...
  assert.equal(
    ...
    '90_System/08_Artifacts/Build Outputs/build-001-build-app-output'
  );
  ...
  assert.match(
    commandCenter,
    /\| build-001 \| build \| app \| Make a tiny timer app from the Lua template \| brief \| Forge \| done \| \[\[90_System\/08_Artifacts\/Build Outputs\/build-001-build-app-output\|artifact\]\] \|/
  );
  ...
  assert.match(runNote, /\[\[90_System\/08_Artifacts\/Build Outputs\/build-001-build-app-output\|Build Output\]\]/);

  const artifact = fs.readFileSync(
    path.join(root, '90_System/08_Artifacts', 'Build Outputs', 'build-001-build-app-output.md'),
    'utf8'
  );
  assert.match(artifact, /type: build-output/);
  assert.match(artifact, /Make a tiny timer app from the Lua template/);
  assert.match(artifact, /node 90_System\/scripts\/check.js/);
