...
const { checkLocalModelAvailability, runLocalTask } = require('./local_model');
...
async function detectAgentAvailability(options = {}) {
  ...
  const claudePath = options.claudePath || env.LUA_CLAUDE_BIN || '/opt/homebrew/bin/claude';
  const codexPath = options.codexPath || env.LUA_CODEX_BIN || '/Applications/ChatGPT.app/Contents/Resources/codex';
  ...
  const local = await checkLocalModelAvailability({ env, fetchImpl: options.fetchImpl });
  return {
    agents: [local.available ? 'local' : '', claudeLoggedIn ? 'claude' : '', codexLoggedIn ? 'codex' : ''].filter(Boolean),
...
function writeWorkerEnv(filePath, values) {
  ...
  try {
    for (const line of fs.readFileSync(filePath, 'utf8').split(/\r?\n/)) {
      ...
      if (['CLAUDE_CODE_OAUTH_TOKEN', 'LUA_LOCAL_MODEL_ENABLED', 'LUA_LOCAL_MODEL', 'LUA_LOCAL_MODEL_URL'].includes(key)) {
        preserved[key] = rest.join('=');
      }
    }
  } catch {}
  const content = [
    ...
    ...Object.entries(preserved).map(([key, value]) => `${key}=${value}`),
    '',
  ].join('\n');
...
async function runWorkerOnce(options = {}) {
  ...
  const availability = options.availability || await detectAgentAvailability({ ...options, env });
  ...
  try {
    const result = task.routeAgent === 'claude'
      ...
      : task.routeAgent === 'local'
        ? await runLocalTask(task, { ...options, env })
...
async function main(options = {}) {
  ...
  if (args.command === 'check') return { ok: true, ...(await detectAgentAvailability({ ...options, env })), paired: Boolean(env.LUA_WORKER_TOKEN) };
