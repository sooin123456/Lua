...
매달 빠져나가는 구독료와 고정비를 귀여운 먼지 캐릭터로 보여주고, 사용자가 안 쓰는 지출을 `재우기` 하면서 절약 행동을 시작하게 만드는 Toss 미니앱.
...
| Project home | 제출 전부터 `02_Projects/TOSS/`에서 별도 관리 |
...
- 먼지방 대표 스크린샷용 화면 상태 확정
- 일반 예시 기준 카피 최종 정리
- Toss 제출 구조로 옮길지 정적 prototype으로 제출할지 결정 후 실행
