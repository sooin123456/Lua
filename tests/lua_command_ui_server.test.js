...
test('runs a Lua Command UI command end-to-end through queue and Atlas router', () => {
  ...
  assert.equal(
    result.run,
    '01_Command Center/01_Commands/Command Runs/lua-ui-20260516-134530-build-app'
  );
  ...
  assert.match(
    commandCenter,
    /\| lua-ui-20260516-134530 \| build \| app \| Lua_template 기반 첫 실제 앱 실행 흐름을 잡아줘 \| plan \| Forge \| routed \| \[\[01_Command Center\/01_Commands\/Command Runs\/lua-ui-20260516-134530-build-app\|run\]\] \|/
  );
  assert.match(
    commandCenter,
  ...
  const runNote = fs.readFileSync(
    path.join(root, '01_Command Center', '01_Commands', 'Command Runs', 'lua-ui-20260516-134530-build-app.md'),
    'utf8'
  );
